...
aSparse = SparseVector(len(aDense), range(len(aDense)), aDense)

bDense = np.array([0., 0., 0., 1.])
bSparse = SparseVector(len(bDense), range(len(bDense)), bDense)
...
def oneHotEncoding(rawFeats, OHEDict, numOHEFeats):
    ...
    """
    subset = {OHEDict[tup]:1.0 for tup in rawFeats if tup in OHEDict}
    return SparseVector(numOHEFeats, subset)
...
def createOneHotDict(inputData):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    distinctFeats = inputData.flatMap(lambda lp: lp).distinct()
    mappedFeats = distinctFeats.zipWithIndex().collectAsMap()
    return mappedFeats
...
def parseOHEPoint(point, OHEDict, numOHEFeats):
    ...
    oheFeat = oneHotEncoding(parsedFeat, OHEDict, numOHEFeats) 
    return LabeledPoint(label, oheFeat)
...
def computeLogLoss(p, y):
    ...
    epsilon = 10e-12
    if p==0: p = p+epsilon
    elif p==1: p = p-epsilon
    if y==1: metric = -log(p)
    elif y==0:metric = -log(1-p)
    return metric
...
def evaluateResults(model, data):
    ...
    """
    ll = data.map(lambda lp: computeLogLoss( getP(lp.features, model.weights, model.intercept), lp.label) )
    return ll.mean()    
...
def parseHashPoint(point, numBuckets):
    ...
    label = point[0]
    parsedFeat = parsePoint(point)
    hashed = hashFunction(numBuckets, parsedFeat)
...
def computeSparsity(data, d, n):
    ...
    """
    #The ratio between non zero elements vs number of elements in the given vector
    #Average of the above number for all the vectors in RDD
