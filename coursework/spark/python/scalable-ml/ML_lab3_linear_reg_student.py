...
sLabels = sorted(onlyLabels.collect())
minYear = min(sLabels)
maxYear = max(sLabels)
...
def calcRMSE(labelsAndPreds):
    ...
    """
    mse = sum( [(l-p)**2 for (l,p) in labelsAndPreds.collect()] )
    rmse = ( mse/labelsAndPreds.count() )**0.5 
    return rmse
...
def getLabeledPrediction(weights, observation):
    ...
    """
    prediction = np.dot(weights.T, observation.features)
...
# TODO: Replace <FILL IN> with appropriate code
firstModel = LinearRegressionWithSGD.train(parsedTrainData, iterations=numIters, step=alpha, regParam=reg,
                regType=regType, intercept=useIntercept, miniBatchFraction=miniBatchFrac)
...
def twoWayInteractions(lp):
    ...
    """
    prl = [reduce(lambda a,b: a*b, pr) for pr in (itertools.product(lp.features, repeat=2)) ]
    return LabeledPoint(lp.label, list(lp.features) + prl)
