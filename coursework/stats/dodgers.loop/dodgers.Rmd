---
title: "dodgers loop sensor"
author: "Ari Kamlani"
date: "December 21, 2015"
output: html_document
---

### Dodgers Loop Sensor Datset
- https://archive.ics.uci.edu/ml/datasets/Dodgers+Loop+Sensor
- Goal: Did a game occur at the stadium or not?
- Alternative Goal : Predict number of vehhicles within the precense of the Loop Region
...
#### Feature Engineering
- Occurence Related: Day, Month Occurence
- Game Related: GameDay, Game InPlay, Game InPlay +/- Epsilon, Game Attendance (Cat/Num), Dodgers Loss, Blowout Game
- Game In Play +/- Epsilon (InPlayEps) becomes the Target Variable (Y)
```{r}
...
#### Exploration
- Game Day Stats: Length of Game, Blowouts, Dodger Game Losses, Attendance
- NumCars as function of Attendance
- NumCars detected per Length of Game:  {None, Average Game Length +/- Epsilon(t)=15 min, Long Games (> 3.4 Hrs)}
- NumCars detected per Game in Session: {None, Game in session, Game in session +/- Epsilon(t)=60 min }
...
#### Subset data and create initial partitions
- As noted in the comments, particularly for time series, samples may be more appropriately taken (CreateTimeSlices)
```{r, message=FALSE}
##### Outliers: TBD
...
#### Feature Estimation
- Initial estimation of feature subset selection based on pearson correlation and RFE
- 4 Features give the lowest RMSE
- Top Variables from RFE: GameInPlay, NumCars, NumGameAttendance, Day, GameLoss
- Most Correlated with Target: GameInPlay, GameDay, NumGameAttendance, GameLoss, NumCars
...
#### PCA Analysis
```{r, message=FALSE}
# Based on Correlation, RFE, and RF Feature Importance, select most relative variables: TBD
library(FactoMineR)
